...
watch(
  ...
  {
    deep: true, // 深度侦听 能侦听到对象深层变化 reactive默认deep
    flush: 'pre' // pre sync post 组件更新前执行/同步执行/组件更新后执行
  }
)

// watch ref需要手动开deep reactive默认开启
watch(
  () => message2.foo.bar.name,
  (newval, oldval) => {
    console.log(newval, oldval)
  }
)
